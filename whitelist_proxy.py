"""
Outbound request whitelist proxy for Prefect.

Import this module **before** `import prefect` to intercept all outbound network
connections and enforce a hostname/CIDR whitelist. Any connection to a destination
not on the whitelist will raise `ConnectionRefusedError`.

Usage:
    import whitelist_proxy  # noqa: F401  (must be first)
    import prefect
...
logger = logging.getLogger("prefect.whitelist_proxy")
